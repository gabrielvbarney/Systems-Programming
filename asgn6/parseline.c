...
void getInput(char *token, Parser *parser){
    ...
    if((in2 = strtok(in1, " ")) == NULL){ /* is there more than one arg*/
        parser->input = open(in1, O_RDONLY, S_IRUSR);
    }
    else{
        parser->input = open(in2, O_RDONLY, S_IRUSR);
    }
}
...
void getOutput(char *token, Parser *parser){
    ...
    if((out2 = strtok(out1, " ")) == NULL){
        parser->output = open(out1, O_CREAT | O_WRONLY | O_TRUNC, S_IWUSR);
    }
    else{
        parser->output = open(out2, O_CREAT | O_WRONLY | O_TRUNC, S_IWUSR);
    }
}
...
/*int main(int argc, const char *argv[]) {
    ...
    errorProcess(cmd_line);
    

    processPipe(cmd_line);
